...
def temp_korriger_last(last, år=1, plot_bool=False):
...
def finn_var_alt_B(last, startdag=0, antall_år=1, ukedag=True):
    """ Finn variasjonskurver med alternativ B. 
        12 variasjonskurver for hverdag (en for hver måned) 
        12 variasjonskurver for helg (en for hver måned) 
    
    Args: 
        last: lastprofil
        startdag: indeks dag last starter på
        antall_år: 
        ukedag: 
    Returns: 
        var_kurve: variasjonskurve for hverdag/helg, hver mnd
    """
    var_kurve = np.zeros(shape=(12, 24))
    for t in range(len(last)):
        år = int(math.floor(t / 8760))
        mnd_ind = get_index_mnd_max(t, år)
        if ukedag:
            if sjekk_hverdag(t, startdag):
                if last[t] > var_kurve[mnd_ind, int(t % 24)]:
                    var_kurve[mnd_ind][int(t % 24)] = last[t]
        else:
            if not sjekk_hverdag(t, startdag):
                if last[t] > var_kurve[mnd_ind, int(t % 24)]:
                    var_kurve[mnd_ind][int(t % 24)] = last[t]
...
def estimer_maks_profil_alt_B(ukedag_var, helg_var, max_P, antall_år=1, startdag=0):
    """ Estimer årlig maks-profil. 
    Args: 
        ukedag_var: variasjonskurve ukedag (normalisert)
        helg_var: variasjonskurve helg (normalisert)
    ...
    for t in range(len(maks_profil)):
        ...
        ind = get_index_mnd_max(t, år)
        if sjekk_hverdag(t, startdag):
            maks_profil[t] = ukedag_var[ind, int(t % 24)] * max_P
        else:
            maks_profil[t] = helg_var[ind, int(t % 24)] * max_P
    return maks_profil
...
def plot_var_kurve_alt_B(var_kurve, ukedag=True):
    """ Plot variasjonskurve for hverdag og helg for alt B
    Args: 
        var_kurve: maks-effekt for hver time på hverdag/helg
        ukedag: indeks om det er hverdag eller helg 
    """
    plt.plot(np.transpose(var_kurve))
    if ukedag:
        plt.title("Variasjonskurver hverdag")
    else:
        plt.title("Variasjonskurver helg")
    plt.legend(
        [
            "Jan",
            "Feb",
...
def plot_est_maks_alt_B(var_kurver_hverdag, var_kurver_helg, startdag=0):
    """ Plot en uke med estimert maks for hver måned
    Args: 
        var_kurver_hverdag: variasjonskurve hverdag
        var_kurver_helg: variasjonskurve helg
        startdag: indeksering av startdag 
    """
    plot_arr = np.zeros(shape=(12, 24 * 7))
    for i in range(12):
        for dag in range(7):
            if dag < 5:
                plot_arr[i][dag * 24 : (dag + 1) * 24] = var_kurver_hverdag[i]
            else:
                plot_arr[i][dag * 24 : (dag + 1) * 24] = var_kurver_helg[i]

    fig, axs = plt.subplots(3, 4)
    axs[0, 0].plot(np.transpose(plot_arr[0]))
    axs[0, 0].set_title("Januar")
    axs[0, 1].plot(np.transpose(plot_arr[1]))
    axs[0, 1].set_title("Februar")
    axs[0, 2].plot(np.transpose(plot_arr[2]))
    axs[0, 2].set_title("Mars")
    axs[0, 3].plot(np.transpose(plot_arr[3]))
    axs[0, 3].set_title("April")
    axs[1, 0].plot(np.transpose(plot_arr[4]))
    axs[1, 0].set_title("Mai")
    axs[1, 1].plot(np.transpose(plot_arr[5]))
    axs[1, 1].set_title("Juni")
    axs[1, 2].plot(np.transpose(plot_arr[6]))
    axs[1, 2].set_title("Juli")
    axs[1, 3].plot(np.transpose(plot_arr[7]))
    axs[1, 3].set_title("August")
    axs[2, 0].plot(np.transpose(plot_arr[8]))
    axs[2, 0].set_title("September")
    axs[2, 1].plot(np.transpose(plot_arr[9]))
    axs[2, 1].set_title("Oktober")
    axs[2, 2].plot(np.transpose(plot_arr[10]))
    axs[2, 2].set_title("November")
    axs[2, 3].plot(np.transpose(plot_arr[11]))
    axs[2, 3].set_title("Desember")

    for ax in axs.flat:
        ax.set(xlabel="Tid [timer]", ylabel="Effekt [kW]")
