...
def plot_maks_effekt(fordeling, simu_antall): 
    ...
    for i in range(3):    
        ...
        print("Med", percentil[i], "%", "sannsynlighet er maks-effekt høyst", P_lim[i],"kW.")
    ...
    values, base, _ = plt.hist( fordeling  , bins = bins, alpha = 0.5, color = "green", range = None)
    ax_bis = ax.twinx()
    values = np.append(values,0)
    ax_bis.plot( base, np.cumsum(values)/ np.cumsum(values)[-1], color='darkorange', marker='o', linestyle='-', markersize = 1, label = label_kumulativ)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel_kum)
    plt.title(tittel)
    for i in range(3):
        plt.axvline(x=P_lim[i], color='k', label='{} %% sannsynlighet'.format(percentil[i]), linestyle='--')
...
def temp_korriger_last(last, døgn_temp = True, plot_bool = False): 
    ...
    if døgn_temp: 
        ...
        #temperatur_gjennomsnitt = np.zeros(shape=(365,1))
        for dag in range(365): 
            #sum_temp_dag = 0
            for t in range(24): 
                temperatur[t+24*dag] = temperatur_array[t+24*dag][3]
                #sum_temp_dag += temperatur[t+24*dag]
            #temperatur_gjennomsnitt[dag] = sum_temp_dag/24
    
    if plot_bool: 
        plot_temperatur(temperatur, normal_temperatur)
    ...
    if døgn_temp: 
        ...
        for dag in np.arange(2,365): 
            avg_last_3_days = (temperatur[dag] + temperatur[dag-1] + temperatur[dag-2]) / 3 
...
def normal_temperatur_døgn():
    ...
    normal_temperatur = np.zeros(shape=(365,1))
    for dag in range(365): 
        sum_temp_dag = 0
...
def find_monthly_max(P_profile): 
    """ Finn månedlig maks
    Args: 
    ...
    for t in range(len(P_profile)): 
        if t < 746:                 # januar
            if P_profile[t] > max_mnd[0]: 
                max_mnd[0] = P_profile[t]
        elif t < 1418 and t > 746: # febraur 
            if P_profile[t] > max_mnd[1]: 
                max_mnd[1] = P_profile[t] 
        elif t < 2161 and t > 1418: # mars
            if P_profile[t] > max_mnd[2]: 
                max_mnd[2] = P_profile[t] 
        elif t < 2881 and t > 2161: # april
            if P_profile[t] > max_mnd[3]: 
                max_mnd[3] = P_profile[t] 
        elif t < 3625 and t > 2881: # mai
            if P_profile[t] > max_mnd[4]: 
                max_mnd[4] = P_profile[t] 
        elif t < 4345 and t > 3625: # juni
            if P_profile[t] > max_mnd[5]: 
                max_mnd[5] = P_profile[t] 
        elif t < 5089 and t > 4345: # juli
            if P_profile[t] > max_mnd[6]: 
                max_mnd[6] = P_profile[t] 
        elif t < 5833 and t > 5089: # august
            if P_profile[t] > max_mnd[7]: 
                max_mnd[7] = P_profile[t] 
        elif t < 6553 and t > 5833: # september
            if P_profile[t] > max_mnd[8]: 
                max_mnd[8] = P_profile[t] 
        elif t < 7298 and t > 6553: # oktober
            if P_profile[t] > max_mnd[9]: 
                max_mnd[9] = P_profile[t] 
        elif t < 8018 and t > 7298: # november
            if P_profile[t] > max_mnd[10]: 
                max_mnd[10] = P_profile[t] 
        elif t > 8018:              # desember
            if P_profile[t] > max_mnd[11]: 
                max_mnd[11] = P_profile[t] 
    return max_mnd 
...
def estimer_maks_profil(max_mnd, ukedag_var, helg_var, max_P, startdag = 0): 
    ...
    """ 
    maks_profil = np.zeros(shape=(8760,))
    for t in range(8760): 
        ind = get_index_mnd_max(t)
        if sjekk_hverdag(t,startdag): 
...
def sorter_avvik(alle_avvik, startdag = 0): 
    ...
    index = 0 
    ind = [-1.00, -0.90, -0.80, -0.70, -0.60, -0.50, -0.40, -0.30, -0.20, -0.10, -0.0, 0.10, 0.20, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90]
...
def plot_pre_mod(last, est_maks, rel_avvik):
    ...
    """ 
    plot_arr = np.zeros(shape=(3,8760))
    plot_arr[0] = last
    plot_arr[1] = est_maks
    plot_arr[2] = rel_avvik
...
def plot_rel_avvik(avvik, startdag = 0): 
    ...
    p2 = plt.bar(pos, avvik_helg, width = 0.75,
                bottom=avvik_hverdag)
    plt.xticks(pos, x)
