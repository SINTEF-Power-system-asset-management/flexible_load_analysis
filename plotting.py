...
def plot_variation_curves(dict_variation_values, str_variation_value_alternative):
    if str.lower(str_variation_value_alternative) == 'a':
        ...
        arr_months = [month_abbr[i] for i in range(1,13)]   # Find a way to do this without calendar
...
def plot_deterministic_load(ts_deterministic_model, str_variation_value_alternative):
    ...
    if str.lower(str_variation_value_alternative) == 'a':
        ...
        int_first_monday_index = first_matching_index(ts_deterministic_model[:,0], lambda dt: dt.weekday() == 0)

        # Finding the second monday
        dt_first = ts_deterministic_model[int_first_monday_index,0]
        int_second_monday_index = first_matching_index(ts_deterministic_model[:,0], lambda dt: dt.weekday() == 0 and dt.date() != dt_first.date())

        fig, ax = plt.subplots(figsize=(10, 6))
        ts = ts_deterministic_model[int_first_monday_index:(int_second_monday_index-1),:]
        arr_time = ts[:,0]
        arr_data = ts[:,1]
        ...
        int_second_monday_index = first_matching_index(ts_deterministic_model[:,0], lambda dt: dt.weekday() == 0 and dt.date() != dt_first_monday.date())
        ...
        while int_months_plotted < 12:
            int_first_monday_of_month_index = first_matching_index(ts_deterministic_model[int_previous_monday_index:,0], lambda dt: dt.weekday() == 0 and dt.month != dt_previous_monday.month)
            dt_first_monday = ts_deterministic_model[int_first_monday_of_month_index,0]
            int_second_monday_index = first_matching_index(ts_deterministic_model[int_previous_monday_index:,0], lambda dt: dt.weekday() == 0 and dt.date() != dt_first_monday.date())

            ts = ts_deterministic_model[int_previous_monday_index+int_first_monday_of_month_index:(int_previous_monday_index+int_second_monday_index-1),:]
            arr_time = ts[:,0]
            arr_days = [dt.strftime("%A %H:%M:%S") for dt in arr_time]
            arr_data = ts[:,1]
            ax.plot(arr_days, arr_data, label=dt_first_monday.strftime("%B"))
...
def plot_selected(dict_config, dict_data_ts, dict_model):       # Bad name
    ...
    if dict_config["modelling"]["chosen_model"] == "toenne":
        ...
        if dict_plot_decider_bool["variation_curves"]: 
            ...
            #plot_deterministic_load(dict_model["biproducts"]["deterministic_model"], str_variation_value_alternative)  # Doesn't work
        ...
        if dict_plot_decider_bool["relative_error"]: plot_timeseries([dict_model["biproducts"]["error_timeseries"]], ["Relative model error"], "Time", "Error", "Relative model error (measurements vs determinisitc model)")
        if dict_plot_decider_bool["relative_error_histogram"]: plot_histogram(dict_model["biproducts"]["error_timeseries"][:,1], "Error", "Occurences", "Relative model error histogram")
